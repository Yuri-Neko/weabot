...
const fakeThumb = fs.readFileSync('./database/fakeThumb.jpg');
...
case 'mega':
  ...
  var file = MGa.fromURL(q);
  await file.loadAttributes();
  fakeSend(`*_Mohon tunggu beberapa menit..._*`, file.name, 'sedang diunduh...');
  var data = await file.downloadBuffer();
  if (/mp4/.test(data)) {
    await sock.sendMessage(from, { document: data, mimetype: "video/mp4", fileName: `${file.name}.mp4` }, { quoted: msg });
  } else if (/pdf/.test(data)) {
    await sock.sendMessage(from, { document: data, mimetype: "application/pdf", fileName: `${file.name}.pdf` }, { quoted: msg });
  }
  break;
case 'tiktok':
  if (!q) {
...
case 'cuaca':
  ...
  await fakeSend(`Tunggu sebentar..`);
